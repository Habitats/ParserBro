...
public abstract class NoCookieParser {
	...
	protected Document genSource(String url) throws NotFoundException {
		...
		while (tries <= Config.HTTP_RETRIES) {
			try {
				source = Jsoup.connect(url).timeout(Config.HTTP_TIMEOUT_SECONDS * 1000).userAgent("Mozilla/5.0 (Windows NT 6.2; WOW64; rv:20.0) Gecko/20100101 Firefox/20.0").ignoreContentType(true).get();
				return source;
			} catch (IOException e) {
