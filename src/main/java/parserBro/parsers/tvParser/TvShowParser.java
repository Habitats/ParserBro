...
public class TvShowParser extends NoCookieParser {
  ...
  private TvShow getShowQuckInfo(String showName) throws Exception {
    Document
        quickSource =
        genSource("http://services.tvrage.com/tools/quickinfo.php?show=" + showName.replaceAll(" ", "+"));
    ...
    TvShow
        tvShow =
        new TvShow(id, title, started, ended, status, runtime, country, classification, network, url, imageUrl);
    tvShow.setLatestEpisode(
        new TvEpisode(latestEpisodeTitle, seasonNumber, latestEpisodeNumber, latestEpisodeDate, tvShow));
